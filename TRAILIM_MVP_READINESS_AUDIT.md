...
> **Audited Branch**: `audit/mvp-readiness` (derived from `main` @ `5ca02de`, informed by verified browser QA on `qa/vs1-manual-browser-e2e`)  
...
4. **Desktop Workspace**: The desktop UI is literally trapped inside a simulated 410px mobile smartphone frame with a fake hardware camera notch.
...
| **RouteVersion** | Firestore `/versions` | Immutable Server Snapshot | Client write completely denied; read gated by approval | Created atomically in transaction by `submitRouteDraft` | 30 unit tests in `versionReviewService.test.mjs` | 2–10 stations limit due to Firestore transaction write ceilings. |
...
| **RouteSession** | Firestore `/routeSessions` | Cloud Functions | Client write denied; active org members can read | Managed by `sessionParticipationService.ts` | 30 unit tests in `sessionParticipationService.test.mjs` | Transitions are forward-only (`open` → `active` → `completed`). |
| **Participation** | Firestore `/participations` | Cloud Functions | Client write denied; student reads own; teacher reads all | Atomic join, progress updates, and completion | Verified in functions tests | Deterministic ID `${sessionId}_${userId}` prevents duplicate joins. |
...
| **privateEvaluation** | Firestore `/privateEvaluation` | Cloud Functions | Student reads only if `completed` + `after_route`; teacher reads always | Stores server evaluation secrets | Verified in rules tests | Subcollection under TaskResponse. |
| **Scoring** | Server Transaction | Server Authoritative | Client cannot supply score | Formula: delta replacement with penalty deduction | Verified in functions tests | Retry penalty calculated server-side. |
...
| **Reveal Policies** | Server Transaction | Server Authoritative | `immediate`: public response; `after_route`: hidden until completion; `never`: redacted | Server redacts `isCorrect`, `feedback`, `pointsAwarded` | Verified in Playwright E2E | UI shows neutral slate "Response submitted" when hidden. |
...
| **Firestore Rules** | Fully tested (41/41 unit tests pass) | **READY FOR DEPLOYMENT** | Run `firebase deploy --only firestore:rules`. |
| **Firestore Indexes** | Defined in `firestore.indexes.json` | **READY FOR DEPLOYMENT** | Run `firebase deploy --only firestore:indexes`. |
| **Cloud Functions** | Fully tested (30/30 unit tests pass) | **READY FOR DEPLOYMENT** (needs env vars) | Remove emulator checks from non-dev functions and run `firebase deploy --only functions`. |
...
| :--- | :--- | :--- | :--- | :--- |
| **Firestore Security Rules** | `firestore.rules.test.mjs` | `npm.cmd run test:rules` | **41 of 41** | Document read/write permissions, role checks, answer key lockouts, draft edits. |
...
| **Cloud Functions Sessions** | `functions/test/sessionParticipationService.test.mjs` | `npm.cmd run test:workflow` | **20 of 20** | Session lifecycle, participation uniqueness, progress updates, response scoring. |
...
| **Playwright Browser E2E** | `scratch/vs1-browser.spec.js` | `npx.cmd playwright test ...` | **2 of 2** | Complete browser flow: Creator → Review → Resubmit → Approve → Play → Resume. |
...
| ID | Problem | User Impact | Evidence / Files | Proposed Direction | Scope | Requires Manual QA? | Execution Type |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **P1-1** | **Desktop App Trapped in Phone Frame** | Desktop creators and teachers must use a tiny 410px mobile column on large screens. | `src/App.tsx`, `src/components/common/BottomNav.tsx` | Replace outer frame with responsive desktop shell (persistent sidebar navigation on screens >= 768px). | **L** | **YES** | **NEEDS PRODUCT DECISION** |
...
- **Server-Authoritative Scoring**: Atomic Firestore transactions that manage points, retry limits, attempt penalties, and reveal policies without trusting client input.
...
- **Comprehensive Backend Test Coverage**: 41 passing security rules unit tests, 30 passing Cloud Functions transaction tests, and 2 passing Playwright browser E2E tests.
